...
func (d *transportDemuxer) deliverPacket(r *Route, protocol tcpip.TransportProtocolNumber, pkt tcpip.PacketBuffer, id TransportEndpointID) bool {
	...
	} else if protocol == header.TCPProtocolNumber && !(isUnicast(r.LocalAddress) && isUnicast(r.RemoteAddress)) {
		// TCP can only be used to communicate between a single source
		// and a single destination; the addresses must be unicast.
		eps.mu.RUnlock()
		return true
