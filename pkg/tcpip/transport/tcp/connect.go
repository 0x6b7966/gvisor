...
func (h *handshake) synRcvdState(s *segment) *tcpip.Error {
	...
	// RFC 793, page 69, states that in the SYN-RCVD state, a sequence
	// number outside of the window causes an ACK with the proper seq
	// number and the endpoint remains in the same state.
	if !s.sequenceNumber.InWindow(h.ackNum, h.rcvWnd) {
