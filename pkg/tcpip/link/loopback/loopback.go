...
func (e *endpoint) WriteRawPacket(pkt tcpip.PacketBuffer) *tcpip.Error {
	// Reject the packet if it's shorter than an ethernet header.
	if pkt.Data.Size() < header.EthernetMinimumSize {
		return tcpip.ErrBadAddress
	}

	// There should be an ethernet header at the beginning of pkt.
	linkHeader := header.Ethernet(pkt.Data.First()[:header.EthernetMinimumSize])
	pkt.Data.TrimFront(len(linkHeader))
